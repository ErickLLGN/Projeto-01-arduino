const byte LED = D1; // Declaração da variável que armazena o pino referente ao LED
...
void loop(void) { 
  ...
  int sensorValue = analogRead(D2); // Declaro a variável que armazena o pino referente aos dados do LDR

  if(sensorValue == 0){  // Teste para identificar se o valor do LDR é igual a zero, se for ele executa as 2 linhas abaixo
